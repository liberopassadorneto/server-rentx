...
let AuthenticateUserUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    // senha está correta?
    const isPasswordMatch = await (0, _bcryptjs.compare)(password, user.password);
    if (!isPasswordMatch) {
      throw new _AppError.AppError("Email or password incorrect");
    }
    ...
    const refresh_token_expires_at = this.dateProvider.addDays(expires_refresh_token_days);
