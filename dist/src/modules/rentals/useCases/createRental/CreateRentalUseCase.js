            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        if (carUnavailable) {
                            throw new AppError_1.AppError("Car is already rented");
                        }
                        return [4 /*yield*/, this.rentalsRepository.findOpenRentalByUser(user_id)];
                    case 2:
                        rentalOpenToUser = _b.sent();
                        if (rentalOpenToUser) {
                            throw new AppError_1.AppError("User already has an open rental");
                        }
                        dateNow = this.dateProvider.dateNow();
                        compare = this.dateProvider.compareInHours(expected_return_date, dateNow);
                        if (compare < minimumHour) {
                            throw new AppError_1.AppError("Expected return date must be at least 24 hours from now");
                        }
